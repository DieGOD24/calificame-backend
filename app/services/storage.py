...
class LocalStorageService(StorageService):
    ...
    def _safe_path(self, path: str) -> Path:
        """Resolve a relative storage path, ensuring it stays inside base_path.

        Defense against path traversal: an attacker-controlled path like
        '../../etc/passwd' must not escape the storage root. We resolve the
        candidate and verify base_path is one of its parents.
        """
        if not path:
        ...
        try:
            ...
            raise ValueError(f"Path traversal detected: {path!r}") from exc
        return candidate
