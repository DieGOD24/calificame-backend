...
class TestAddEnrollment:
    def test_add_single_student(self, client: TestClient, test_class: Class, auth_headers: dict) -> None:
    ...
    def test_add_student_without_email(self, client: TestClient, test_class: Class, auth_headers: dict) -> None:
        response = client.post(
            f"/api/v1/classes/{test_class.id}/enrollments",
            headers=auth_headers,
            json={
    ...
    def test_invalid_data_422(self, client: TestClient, test_class: Class, auth_headers: dict) -> None:
        response = client.post(
            f"/api/v1/classes/{test_class.id}/enrollments",
            headers=auth_headers,
            json={
...
class TestBulkEnroll:
    ...
    def test_bulk_upload_csv(self, client: TestClient, test_class: Class, auth_headers: dict) -> None:
    ...
    def test_bulk_upload_missing_columns(self, client: TestClient, test_class: Class, auth_headers: dict) -> None:
        file_tuple = self._make_csv_upload(
            [
    ...
    def test_bulk_upload_empty_file(self, client: TestClient, test_class: Class, auth_headers: dict) -> None:
        file_tuple = self._make_csv_upload(
            [
                ["nombre", "codigo"],
            ]
    ...
    def test_bulk_upload_non_owner_forbidden(self, client: TestClient, test_class: Class, auth_headers_2: dict) -> None:
        file_tuple = self._make_csv_upload(
            [
                ["nombre", "codigo"],
                ["Alice Smith", "STU-301"],
...
class TestListEnrollments:
    def test_list_students(
        ...
        response = client.get(f"/api/v1/classes/{test_class.id}/enrollments", headers=auth_headers)
        assert response.status_code == 200
        data = response.json()
        assert len(data) == 1
    ...
    def test_list_non_owner_non_enrolled_forbidden(
        ...
        response = client.get(f"/api/v1/classes/{test_class.id}/enrollments", headers=auth_headers_2)
        assert response.status_code == 403
...
class TestRemoveEnrollment:
    ...
    def test_remove_nonexistent_404(self, client: TestClient, test_class: Class, auth_headers: dict) -> None:
        response = client.delete(
